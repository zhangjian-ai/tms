...
function base (proxyHost, proxyPort) {
  return `http://${proxyHost}:${proxyPort}`
}

export async function listApps (proxyHost, proxyPort, serial) {
  const res = await fetch(`${base(proxyHost, proxyPort)}/devices/${serial}/apps`)
  const data = await res.json().catch(() => ({}))
  if (!res.ok) throw new Error(data.error || `获取应用列表失败(${res.status})`)
  return data.apps || []
}

export async function uninstallApp (proxyHost, proxyPort, serial, id) {
  const res = await fetch(`${base(proxyHost, proxyPort)}/devices/${serial}/apps/uninstall`, {
    method: 'POST',
...
export function installApp (proxyHost, proxyPort, serial, file, onProgress) {
  return new Promise((resolve, reject) => {
    const url = `${base(proxyHost, proxyPort)}/devices/${serial}/apps/install?filename=${encodeURIComponent(file.name)}`
